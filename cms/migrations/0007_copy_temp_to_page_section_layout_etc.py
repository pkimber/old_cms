...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for section in orm.TempSection.objects.all():
            clean_and_save(orm.Page(
                name=section.name,
                order=0,
                slug=slugify(unicode(section.name)),
            ))
        pending = clean_and_save(orm['moderate.ModerateState'](
            name='Pending',
            slug=slugify(unicode('pending')),
        ))
        published = clean_and_save(orm['moderate.ModerateState'](
            name='Published',
            slug=slugify(unicode('published')),
        ))
        body = clean_and_save(orm.Layout(
            name='Body',
        ...
        for simple in orm.TempSimple.objects.all():
            section = orm.Section.objects.get(
                page__slug=simple.section.name,
                layout=body,
            )
            container = clean_and_save(orm.Container(
                section=section,
            ))
            moderate_state = pending
            if simple.moderated:
                moderate_state = published
            clean_and_save(orm.Content(
                container=container,
                order=simple.order,
                title=simple.title,
    ...
    models = {
        ...
        u'cms.content': {
            'Meta': {'ordering': "['section__page__name', 'order', 'moderate_state__slug']", 'unique_together': "(('container', 'moderate_state'),)", 'object_name': 'Content'},
